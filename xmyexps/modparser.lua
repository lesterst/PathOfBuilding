...
local function scan(line, patternList, plain)
	for key, value in pairs(patternList) do
        -- print(key)
        break  -- Exit the loop after printing the first key-value pair
    end
	...
	for pattern, patternVal in pairs(patternList) do
		...
		if index and (not bestIndex or index < bestIndex or (index == bestIndex and (endIndex > bestEndIndex or (endIndex == bestEndIndex and #pattern > #bestPattern)))) then
			...
			local first10Chars = string.sub(pattern, 1, 10)
			print(first10Chars, bestIndex,bestEndIndex, bestPattern, bestVal, bestStart, bestEnd, inspect(bestCaps))
		end
	end
	if bestVal then
...
local function parseMod(line, order)
	...
	local lineLower = line:lower()
	-- print("parseMod line", line)
	-- print("scan specialModList")
	local specialMod, specialLine, cap = scan(line, specialModList)
	-- print(specialMod,specialLine,"test")
	if specialMod and #specialLine == 0 then
		print("specialMod found")
...
local function parseLine(line, isComb)
...
-- local exampleLine = "minions explode when reduced to low life, dealing 33%% of their maximum life as fire damage to surrounding enemies"
local exampleLine = "+20% to Strength and Dexterity" -- standard mod form to base
-- local exampleLine = "enemies you kill have a 30% chance to explode, dealing a tenth of their maximum life as fire damage" -- specialMod returns a function
-- local exampleLine = "maximum life becomes 1, immune to chaos damage" -- specialmod returns a table
-- local exampleLine = "always poison on hit while using a flask" --  = { mod("PoisonChance", "BASE", 100, { type = "Condition", var = "UsingFlask" }) },
-- local exampleLine = "attacks used by totems have" --preflag returns a table
-- print(inspect(exampleLine))
print(parseLine(exampleLine))